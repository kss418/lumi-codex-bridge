...
class QueueTests(unittest.TestCase):
    def test_accepts_input_during_generation_and_drains_before_shutdown(self):
        ...
        def generate(thread_id, text, effort=None):
            if text == 'direct':
        ...
        sink = io.StringIO()
        serve_lines(StdioSession(c), source(), sink, queue_size=1)
    ...
    def test_bounded_queue_preserves_all_requests_and_continues_after_error(self):
        ...
        c.send_message.side_effect = generate
        sink = io.StringIO()
        serve_lines(StdioSession(c), (request(i) for i in range(100)), sink, queue_size=2)
