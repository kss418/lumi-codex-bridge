...
캐릭터 우클릭 → **꼬미와 대화**에서 텍스트를 입력할 수 있습니다. 답변은 대화창과 해당 꼬미의 말풍선에 표시합니다.


## 꼬미와 텍스트 대화

- 캐릭터 우클릭 → **꼬미와 대화**를 선택하고 메시지를 입력한 뒤 Enter 또는 **보내기**를 누릅니다.
- 대화창을 열 때 저장된 모델·추론 강도를 읽습니다. 변경한 설정을 적용하려면 대화창을 닫고 다시 여세요.
- 첫 메시지에서 Python 브릿지를 시작하고, 창을 열어 둔 동안 같은 Codex 대화를 재사용합니다.
- 캐릭터 인스턴스마다 별도 창과 대화를 사용합니다. 여러 창은 각각 Python/Codex 프로세스를 사용합니다.
- 답변 생성 중에는 전송을 잠시 막습니다. UI는 백그라운드 작업으로 계속 반응합니다.
- 응답 ID를 확인한 뒤 대화창과 해당 캐릭터의 말풍선(15초)에 답변을 표시합니다.
- 창을 닫거나 플러그인을 종료하면 해당 Python/Codex 프로세스를 정리합니다. 닫은 대화의 복원은 아직 없습니다.
- 오류나 시간 초과 후에는 창을 닫고 다시 여세요. 자동 재전송은 하지 않습니다.
- 현재는 일반 텍스트 대화이며, 캐릭터 페르소나 자동 전달·화면 인식·음성 합성은 아직 없습니다.
